...
def snow_vehicle_usage(connection, file_name, stage, file_format, table_name): 
    ...
    (
        ...
        .select(
            ...
            col('"c2"').alias("vehicle_id"),
            col('"c3"').alias("license_plate"),
            col('"c4"').alias("vehicle_type"),
            to_decimal(col("$5"), 12, 2).alias("mileage_km"),
            col('"c6"').alias("fuel_type"),
            to_decimal(col("$7"), 10, 2).alias("fuel_consumed_liters"),
            to_decimal(col("$8"), 10, 2).alias("maintenance_cost_usd"),
            col('"c9"').cast("int").alias("breakdowns_count"),
            col('"c10"').alias("maintenance_notes")
        )
        .write.mode('append')
        .save_as_table(table_name)
    )

def populate_vehicle_usage(
    stage:str,
    stream:str,
    table_name:str,
    file_format:str,
    ...
    with create_snowpark_conn(account_name, user, password, role, wh, db, schema) as snow_conn:
        ...
        for file in files:
            snow_vehicle_usage(connection=snow_conn, file_name=file, stage=stage, file_format=file_format, table_name=table_name)
